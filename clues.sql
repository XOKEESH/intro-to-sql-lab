...
--A = Holy See (Vatican City State) |       1000
...
FROM cities
JOIN countries ON cities.countrycode = countries.code
WHERE cities.name LIKE 'Ser%' 
AND countries.region = 'South America';

--A = City: Serra Country: Brazil
...
-- then I used: SELECT name FROM cities WHERE id = 211;
-- to get:
...
--A = Santa Monica, CA
-- My group couldnt get the psql -f clues.sql commant to work until we finished so we had to add the answers here than comment them out. Thanks Ashley for letting us know to add \c world at the top of the file. :)
